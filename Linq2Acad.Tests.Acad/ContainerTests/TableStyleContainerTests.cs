    ...
    public void TestCreateTableStyle()
    {
      ...
      try
      {
        using (var db = AcadDatabase.Active())
        {
          ...
          var ok = Check.Dictionary(db.Database, db.Database.TableStyleDictionaryId, dict => dict.Contains("NewTableStyle"));
          if (!ok) { notifier.TestFailed("TableStyle dictionary does not contain an element with name 'NewTableStyle'"); return; }

          ok = Check.DictionaryIDs(db.Database, db.Database.TableStyleDictionaryId, ids => ids.Any(id => id == newTableStyle.ObjectId));
