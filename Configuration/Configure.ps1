...
if (Test-Path $folder)
{
  ...
  (Get-Content -Encoding UTF8 ..\AcadTestRunner\AcadTestRunner.dll.config) | ForEach-Object { $_ -replace "{AcadRootDir}", "$folder" } | Set-Content -Encoding UTF8 ..\AcadTestRunner\AcadTestRunner.dll.config
